...
if (count($_POST)) {
	...
	try {
		...
		// Woohoo! Register ;)
		$result = $server->loginServer->register($username, $password, $confirm, $email, $code);
		...
		if ($result) {
			if (Flux::config('RequireEmailConfirm')) {
				...
				$link = $this->url('cpaccount', 'confirm', array('_host' => true, 'code' => $code, 'user' => $username, 'login' => $name));
